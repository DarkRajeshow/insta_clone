...
export default function PostCard({ post, index, userId }) {
    ...
    return (
        <Link to={`/post/${post._id}`} key={index} className={`relative post aspect-square bg-gray-900 rounded-sm group cursor-pointer ${isVisible && !loading ? `visible animate-in fade-in-5` : "invisible"}`}
        >
            {post.type === "video" ?
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-light absolute top-4 right-4 z-40">
            ...
            <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all'>

                <div className="likes flex items-center flex-col justify-center ">
                    {(userId && (post.likes.indexOf(userId) >= 0)) ?
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-10 h-10 cursor-pointer text-red-500">
